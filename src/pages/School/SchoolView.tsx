    ...
    return (
        <div className="overflow-x-auto">
            <table className="w-full border-collapse text-xs">
                <thead>
                    <tr>
                        <th className="p-1 text-slate-500 dark:text-slate-400 text-[10px] font-normal w-12 border-b border-slate-300 dark:border-slate-700"></th>
                        {sortedDates.map(dateKey => (
                            <th key={dateKey} className="p-1 text-center border-b border-slate-300 dark:border-slate-700 min-w-[60px]">
                                <div className="text-slate-600 dark:text-slate-300 font-bold text-sm">{format(new Date(dateKey), 'EEE', { locale: de })}</div>
                                <div className="text-slate-400 dark:text-slate-500 text-[10px]">{format(new Date(dateKey), 'dd.MM.', { locale: de })}</div>
                            </th>
                        ))}
                    </tr>
                </thead>
                <tbody>
                    {sortedPeriods.map((period, pIdx) => {
                        ...
                        return (
                            <tr key={period} className={pIdx % 2 === 0 ? 'bg-slate-200/20 dark:bg-slate-800/20' : ''}>
                                <td className="p-1 text-center border-r border-slate-300 dark:border-slate-700 text-[10px] text-slate-500 dark:text-slate-400 align-top">
                                    <div className="font-bold text-slate-600 dark:text-slate-300">{pIdx + 1}</div>
                                    <div>{startTime}</div>
...
export const SchoolView: React.FC = () => {
    ...
    if (loading && students.length === 0) {
        return (
            <div className="h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 space-y-4 animate-pulse">
                <GraduationCap className="w-16 h-16 opacity-50" />
    ...
    return (
        <div className="h-full w-full p-4 flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between bg-slate-200/50 dark:bg-slate-800/50 p-2 rounded-lg border border-slate-300 dark:border-slate-700">
                ...
                <div className="text-slate-700 dark:text-slate-200 font-medium cursor-pointer hover:text-slate-900 dark:hover:text-white transition-colors" onClick={handleResetWeek}>
                    <span className="hidden sm:inline">Woche vom </span>
                    {format(currentDate, 'dd.MM.yyyy', { locale: de })}
                </div>

                <div className="flex items-center gap-1">
                    <button onClick={handleNextWeek} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-700 rounded text-slate-600 dark:text-slate-300 transition-colors flex items-center gap-2">
                        <span className="hidden sm:inline">Woche &gt;</span>
                        <Clock className="w-5 h-5" />
                    </button>
                    <div className="w-px h-6 bg-slate-300 dark:bg-slate-700 mx-1"></div>
                    <button onClick={refresh} title="Daten aktualisieren" className="p-2 hover:bg-slate-200 dark:hover:bg-slate-700 rounded text-slate-600 dark:text-slate-300 hover:text-blue-400 transition-colors">
            ...
            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 overflow-hidden">
                {students.map((student, idx) => {
                    ...
                    return (
                        <div key={idx} className="bg-slate-200/50 dark:bg-slate-800/50 rounded-xl border border-slate-300 dark:border-slate-700 flex flex-col overflow-hidden shadow-lg backdrop-blur-sm h-full">
                            ...
                            {/* Tabs */}
                            <div className="flex bg-white/40 dark:bg-slate-900/40 p-1 border-b border-slate-200 dark:border-slate-800">
                                {[
                            ...
                            {/* Content */}
                            <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-slate-200/20 dark:bg-slate-800/20">
                                    ...
                                    <div className="space-y-4">
                                        {(!student.messages || student.messages.length === 0) ? (
                                            ...
                                            student.messages.map((msg, mIdx) => (
                                                <div key={mIdx} className="bg-slate-300/40 dark:bg-slate-700/40 p-4 rounded-lg border-l-4 border-blue-500 shadow-sm">
                                                    <div className="flex justify-between items-start mb-2">
                                                        <h4 className="font-bold text-slate-700 dark:text-slate-200 text-sm">{msg.title}</h4>
                                                    ...
                                                    <p className="text-slate-600 dark:text-slate-300 text-xs leading-relaxed opacity-90">{msg.body}</p>
                                                </div>
                                            ))
                                        )}
                                    </div>
                ...
                {students.length === 0 && !loading && !error && (
                    <div className="col-span-2 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 mt-20">
