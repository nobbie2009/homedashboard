...
const StarTrail: React.FC<{ filled: number; total: number; color?: string }> = ({ filled, total, color }) => {
    ...
    return (
        <div className="w-full">
            <div className="relative h-7 rounded-full overflow-hidden bg-slate-800/80 border border-slate-700/60">
                <div
...
const FloatingStar: React.FC<{ delay: number; left: string }> = ({ delay, left }) => (
    <div
        className="absolute text-yellow-400/20 animate-pulse pointer-events-none"
        style={{
...
const RewardBoard: React.FC = () => {
    ...
    const [claimOverlay, setClaimOverlay] = useState<string | null>(null); // kidId or 'shared'
    ...
    return (
        <div className="h-full w-full overflow-y-auto relative">
            ...
            <div className="relative z-10 max-w-4xl mx-auto px-4 py-6 space-y-6">
                ...
                <div
                    className="relative rounded-2xl overflow-hidden border border-yellow-600/30"
                    style={{
                        background: 'linear-gradient(135deg, rgba(120, 53, 15, 0.4), rgba(30, 30, 50, 0.6))',
                    }}
                >
                    ...
                    <div className="relative flex items-center gap-5 p-5">
                        {rewards?.rewardImage ? (
                            <img
                                ...
                                alt="Belohnung"
                                className="w-20 h-20 rounded-xl object-cover border-2 border-yellow-600/40 shadow-lg flex-shrink-0"
                            />
                        ) : (
                            <div className="w-20 h-20 rounded-xl bg-yellow-900/30 border-2 border-yellow-700/30 flex items-center justify-center flex-shrink-0">
                                <Gift className="w-10 h-10 text-yellow-600/60" />
                            </div>
                        )}
                        <div className="flex-1 min-w-0">
                            <p className="text-xs font-bold uppercase tracking-widest text-yellow-600/80 mb-1">
                                Aktuelle Belohnung
                            </p>
                    ...
                    <div className="space-y-3">
                        <div className="bg-slate-800/50 rounded-2xl border border-slate-700/40 p-6">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h2 className="text-xl font-bold text-white">Gemeinsamer Fortschritt</h2>
                            </div>
                            <StarTrail
                            ...
                            {isGoalReached() && (
                                <div className="mt-5 p-4 rounded-xl bg-gradient-to-r from-yellow-600/20 via-amber-500/20 to-orange-600/20 border border-yellow-500/40 text-center animate-pulse">
                        ...
                        {kids.map(kid => {
                            ...
                            return (
                                <div
                                    ...
                                    style={{
                                        background: reached
                                            ? `linear-gradient(135deg, ${kid.color}15, rgba(234,179,8,0.08))`
                                            : `linear-gradient(135deg, ${kid.color}10, rgba(30,41,59,0.5))`,
                                    }}
                                >
                                    <div className="p-5">
                                        <div className="flex items-center gap-4 mb-4">
                                            ...
                                            <div className="flex-1">
                                                <h3 className="text-xl font-black text-white">{kid.name}</h3>
                                        ...
                                        {reached && (
                                            <div className="mt-4 text-center">
                                                <p className="text-lg font-black text-yellow-300 mb-2 animate-pulse">
                                                    Geschafft!
                                                </p>
                                                <button
                        ...
                        <div className="divide-y divide-slate-800/60 max-h-64 overflow-y-auto">
                            {history.map(entry => {
                                ...
                                return (
                                    <div
                                        key={entry.id}
                                        className="flex items-center gap-3 px-5 py-3 hover:bg-slate-800/30 transition"
                                    >
                                        <div
                                            className="w-3 h-3 rounded-full flex-shrink-0"
                                            style={{ backgroundColor: kid?.color || '#64748b' }}
                                        />
                                        <span className="font-bold text-white text-sm">{entry.kidName}</span>
                                        <span className="text-slate-400 text-sm flex-1 truncate">{entry.taskLabel}</span>
