    ...
    return (
        <div className="flex flex-col p-4 bg-slate-200/60 dark:bg-slate-800/60 rounded-xl backdrop-blur-md shadow-lg w-full h-full border border-slate-300 dark:border-slate-700 overflow-hidden">
            ...
            <div className="flex-1 flex flex-col justify-around space-y-2">
                        ...
                        <div key={kid.id} className="flex items-center bg-slate-300/40 dark:bg-slate-700/40 rounded-lg p-1.5">
                            ...
                            <div className="flex flex-col items-center justify-center w-20 mr-3 border-r border-slate-300/50 dark:border-slate-600/50 pr-2">
                                {kid.photo ? (
                                    ...
                                    <div className="w-8 h-8 rounded-full flex items-center justify-center text-slate-900 dark:text-white font-bold text-xs border-2" style={{ backgroundColor: kid.color, borderColor: 'rgba(255,255,255,0.2)' }}>
                            ...
                            <div className="flex-1 flex flex-wrap gap-2 items-center">
                                {kidTasks.length > 0 ? (
                                    kidTasks.map(task => (
                                        <div key={task.id} className="flex items-center gap-1.5 px-2 py-1 bg-slate-300/50 dark:bg-slate-600/50 rounded text-slate-700 dark:text-slate-200">
                                    ...
                                    <span className="text-slate-400 dark:text-slate-500 text-[10px] italic ml-2">Frei!</span>
                                )}
                            </div>
                        </div>
                    );
                ...
                {(!kids || kids.length === 0) && (
                    <div className="text-slate-400 dark:text-slate-500 text-sm italic text-center">Keine Kinder konfiguriert</div>
                )}
            </div>
        </div>
    );
