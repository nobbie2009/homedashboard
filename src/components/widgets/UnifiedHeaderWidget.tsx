    ...
    return (
        <div className="flex items-center justify-between p-6 bg-slate-800/60 rounded-xl backdrop-blur-md shadow-lg w-full h-full border border-slate-700 text-white relative overflow-hidden">
            ...
            <div className="flex flex-col justify-center min-w-[200px]">
                ...
                {config.showSeconds && (
                    <div className="text-xl text-slate-500 font-mono mt-[-5px] pl-2">
                        {format(time, ':ss')}
                    </div>
                )}
            </div>
                        ...
                        <div className="flex space-x-6 mt-2">
                            {weather.forecast.map((day, idx) => (
                                <div key={idx} className="flex flex-col items-center">
                                    <span className="text-slate-500 text-xs mb-1">{day.day}</span>
                                    {getWeatherIcon(day.code, "w-5 h-5 text-slate-300 mb-1")}
                                    <span className="text-sm font-semibold">{day.tempMax}° <span className="text-slate-600 text-xs">{day.tempMin}°</span></span>
                                </div>
                            ))}
                        </div>
                    </>
            ...
            <div className="flex flex-col items-end justify-center min-w-[200px]">
                ...
                <div className="text-3xl text-slate-200 font-light">
